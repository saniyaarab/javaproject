...

{
	...
	private void AddControls( )
	{
		try
		{
			...
			m_txtDateOfBirth.setHorizontalAlignment( JTextField.RIGHT ); //Right align
	...
	public void windowOpened( WindowEvent weSource )
	{
		try
		{
			...
			{
				//No Warn user and ...
				CMessageBox.Show( this, "Unable to load the State list.\n" + 
										"The Form will now close.", "Edit The Player",
										enuIconType.Error );
			}
		}
	...
	private boolean IsValidData( )
	{
		...
		try
		{
			...
			//HomePhoneNumber if not blank the validate
	...
	private boolean SaveData( )
	{
		...
		try
		{
			...
			udtNewPlayer.intPlayerID 		= m_intPlayerToEditID; // don't know it yet so set it to 0
			...
			blnResult = CDatabaseUtilities.EditPlayerInDatabase( udtNewPlayer );		
		}
		catch ( Exception excError )
		{
			//Display Error Message
